...
class WorkerTransport {
  ...
  getFieldObjects() {
    if (!this._getFieldObjectsPromise) {
  ...
  getMetadata() {
    if (!this.metadataPromise) {
      this.metadataPromise = this.messageHandler.sendWithPromise("GetMetadata", null).then(results => {
        return {
          info: results[0],
...
const _privateObjs = new WeakMap();

class PDFObjects {
  constructor() {
    // 传统写法1：用 _ 前缀约定表示私有（外部仍可访问）
    this._objs = Object.create(null);

    // 传统写法2：用 WeakMap 存储私有数据（更接近真正的私有）
    _privateObjs.set(this, {
  ...
  // 传统写法1：用 _ 前缀表示私有方法
  _ensureObj(objId) {
    const objs = this._objs; // 或 _privateObjs.get(this).objs
  ...
  get(objId, callback = null) {
    if (callback) {
      const obj = this._ensureObj(objId); // 调用私有方法
      obj.capability.promise.then(() => callback(obj.data));
      return null;
    }

    const obj = this._objs[objId]; // 访问私有字段
...
class Metadata {
  ...
  get(name) {
    if (!this.metadataMap.has(name)) {
      this.metadataMap.get(name) = null
    }
